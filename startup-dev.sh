...
if $build || [ -z "$(docker images -q $image_name)" ]; then
    cd "$lard_path" \
        && echo -e "建立並啟動容器..." \
        && docker-compose up --build -d laravel-echo-server nginx mariadb phpmyadmin redis php-worker workspace
else
    cd "$lard_path" \
        && echo -e "啟動容器..." \
        && docker-compose up -d laravel-echo-server nginx mariadb phpmyadmin redis php-worker workspace
fi
